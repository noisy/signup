  ...
  <div class="Box__container">
    <div class="Box__inner">
      <div class="Code">
        ...
        <p style="margin-bottom:10px;">Didn't get the SMS? <button 
          :disabled="clicked_resend" 
          style="margin-left:10px;" 
          class="Btn__light" 
          @click="resendCode()">Resend</button></p>
        <p style="margin-bottom:10px;">Wrong number? <a 
          class="clickable_link" 
          @click="changeNumber()">Change Number</a></p>
        <div>
          <button 
            :disabled="input_code.length < 4 || input_code.length > 4 || clicked_continue" 
            class="Btn__blue" 
...
export default {
  ...
  methods: {
      ...
      this.$store
        ...
        .then(result => {
          if (result) {
            ...
            this.$store.dispatch("resendPhone").then(response => {
              if (response.status !== 200) {
                this.$notify({
                  group: "main",
                  text: response.data.message,
    ...
    submitCode() {
      this.$store
        .dispatch("checkCookie", { cookie_name: "s_t", timeout: 60 })
        .then(result => {
          if (result) {
            this.$store
              .dispatch("confirmPhone", { code: this.input_code })
              .then(response => {
                if (response.status === 200) {
                  this.$store.dispatch("changeCookie", {
                    cookie_name: "s_t",
            ...
            this.$notify({
              group: "main",
              text: "You have tried too many times - please wait 1 Minute",
              type: "error"
            });
          }
        });
