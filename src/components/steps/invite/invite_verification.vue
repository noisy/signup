  ...
  <div class="Box__container">
    <div class="Box__inner">
      ...
      <div style="margin-bottom:2px;"  class="Code__Input">
        <img src="./../../../assets/ic_email.svg"><input class="Input__utopian" style="margin-right:10px; border: 1px solid #E0E2E5;" placeholder="Invitation code" v-model="input_invite_code">
      </div>
      <div>
        <button style="margin-top:20px;" class="Btn__blue" @click="sendCode()">CONTINUE</button>
      </div>
      <div style="height:24px;width:100%;"><p class="text__error" v-show="this.input_error">{{ input_error }}</p></div>
      <p style="text-align: center;margin-bottom:10px;"><a class="clickable_link" @click="userWithoutCode()">Don't have an invitation code? </a></p>
    </div>
  <Login/>
  </div>
</template>
  ...
  methods: {
    sendCode() {
      this.$store.dispatch('checkCookie', { cookie_name: 's_s', timeout: 60 * 10 })
        .then(result => {
          if(result) {
            this.$store.commit('setInviteCode', { invite_code: this.input_invite_code })
            this.$store.dispatch('confirmInviteCode')
            .then(response => {
