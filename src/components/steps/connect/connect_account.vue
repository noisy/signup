<template>
  <div class="Box__container">
    <div class="Box__inner">
      ...
      <div class="options">
        <button class="Btn__blue" @click="noAccount()">I DON'T HAVE AN STEEM ACCOUNT</button>
        <button class="Btn__blue" @click="steemConnect()">CONNECT MY EXISTING STEEM ACCOUNT</button>
      </div>
    </div>
  <Login/>
  </div>
...
export default {
  ...
  methods: {
    noAccount() {
      if(!this.user.social_verified && !this.user.sms_verified && !this.user.email_verified) { this.$router.push('/verify_invite') }
      else if((this.user.social_verified || this.user.sms_verified) && this.user.email_verified) { this.$router.push('/pick_account') }
      else if(!this.user.email_verified) { this.$router.push('/verify_mail') }
