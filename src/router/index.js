...
const router = new Router({
  ...
  routes: [
    ...
    },
    {
      ...
      name: "save_key",
      component: saveKey,
      meta: { requiresSignIn: true, requiresAccountName: true }
    },
    {
      path: "/phone/success",
      name: "success_phone",
...
router.beforeEach((to, from, next) => {
  if (to.matched.some(record => record.meta.requiresSignIn)) {
    ...
    if (!store.getters.currentUserObject.social_name && chosen_acc_name) {
      next({ path: "/" });
    } else {
      next();
    }
